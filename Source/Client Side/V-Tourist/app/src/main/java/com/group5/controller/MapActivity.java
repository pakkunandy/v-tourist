...
public class MapActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private class DownloadPlaceTask extends AsyncTask<LatLng, Void, Void> {
        @Override
        protected Void doInBackground(LatLng... params) {
            try {
                placeList =  PlaceServices.getPlacesListByGeo(params[0].latitude,params[0].longitude,1.0);
        ...
        protected void onPostExecute(Void places) {
            ...
            for(int i = 0; i< placeList.size(); i++){
                googleMap.addMarker(new MarkerOptions().icon(BitmapDescriptorFactory.fromResource(R.drawable.ic_location_default)).anchor(0.0f,
                        0.0f).position(new LatLng(placeList.get(i).getLatitude(),placeList.get(i).getLongitude())).title(placeList.get(i).getPlaceName() + " => Đi đến")).showInfoWindow();
                ...
                googleMap.setOnInfoWindowClickListener(new GoogleMap.OnInfoWindowClickListener() {
                    ...
                    public void onInfoWindowClick(Marker marker) {
                        ...
                        {
                            if (placeList.get(j).getLatitude() == marker.getPosition().latitude && placeList.get(j).getLongitude() == marker.getPosition().longitude)
                            {
